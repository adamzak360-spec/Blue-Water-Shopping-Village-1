...
const slides = [
  { title: 'Reliable Premium Marketplace', summary: 'Company mission, platform promise, and operating model.', visual: '/campaign-assets/reliable-hero-shopping.jpg' },
  ...
  { title: 'Seller tools that support real operations', summary: 'Storefront, catalogue, orders, chat, delivery, and payouts.', visual: '/campaign-assets/reliable-storefront-team.jpg' },
  ...
  { title: 'Trust is built into payments and payouts', summary: 'Settlement discipline, wallet accumulation, and idempotent transfers.', visual: '/campaign-assets/reliable-conference-stage.jpg' },
  { title: 'Familiar communication, accountable security', summary: 'Chat receipts, unread awareness, offline retry, and sign-in protection.', visual: '/campaign-assets/reliable-hero-shopping.jpg' },
  { title: 'Administration is the control centre', summary: 'Governance across stores, catalogue, finance, security, and knowledge.', visual: '/campaign-assets/reliable-exhibition-wall.jpg' },
  { title: 'Reliable: discover with confidence', summary: 'Closing promise for meetings and conferences.', visual: '/campaign-assets/reliable-pullup-banner.jpg' },
]
...
export default function ReliableOperationsWorkspace() {
  ...
  const exportHtml = useMemo(() => {
    ...
    const sections = slides.map((slide, index) => `<section class="slide"><img class="slide-visual" src="${assetBase}${slide.visual}" alt=""><div class="slide-overlay"></div><img class="logo" src="${logo}" alt="Reliable logo"><div class="eyebrow">RELIABLE PREMIUM MARKETPLACE · ${String(index + 1).padStart(2, '0')}</div><h1>${slide.title}</h1><p>${slide.summary}</p><div class="rule"></div><small>Company and platform operations</small></section>`).join('')
  ...
  const handleLogo = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setLogoUrl(await logoToDataUrl(file))
  }
  ...
  return (
    <section className="reliable-operations-workspace">
      <header className="operations-header">
        ...
        <div className="operations-brand"><img src={logoUrl} alt="Reliable logo preview" /><span>Brand-ready workspace</span></div>
      </header>
