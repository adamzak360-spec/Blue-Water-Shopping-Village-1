...
export default function AdminAds() {
  ...
  const saveAd = async (event: React.FormEvent) => {
    ...
    if (!form.advertiser_id || !form.campaign_name.trim() || !form.headline.trim() || !form.destination_url || !form.starts_at || !form.ends_at) { setMessage('Complete the advertiser, campaign, headline, destination, and schedule fields.'); return }
    if (!isValidHttpsUrl(form.destination_url) || (form.image_url && !isValidHttpsUrl(form.image_url))) { setMessage('Advertisement and image URLs must use HTTPS.'); return }
    if (new Date(form.ends_at) <= new Date(form.starts_at)) { setMessage('The end date must be after the start date.'); return }
    setBusy(true)
  ...
  return <section className="admin-ads animate-fade-in">
    ...
    <form className="ads-form" onSubmit={saveAd}><h3>Create advertisement</h3><div className="ads-form-grid">
      <select required value={form.advertiser_id} onChange={(e) => setForm({ ...form, advertiser_id: e.target.value })}><option value="">Select advertiser</option>{advertisers.map((advertiser) => <option key={advertiser.id} value={advertiser.id}>{advertiser.name} ({advertiser.advertiser_type})</option>)}</select>
      <input required placeholder="Campaign name" value={form.campaign_name} onChange={(e) => setForm({ ...form, campaign_name: e.target.value })} />
      ...
      <input required placeholder="Headline" maxLength={180} value={form.headline} onChange={(e) => setForm({ ...form, headline: e.target.value })} />
      <input placeholder="Description" maxLength={500} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} />
      <input type="url" placeholder="Image URL (HTTPS, optional)" value={form.image_url} onChange={(e) => setForm({ ...form, image_url: e.target.value })} />
      <input required type="url" placeholder="Destination URL (HTTPS)" value={form.destination_url} onChange={(e) => setForm({ ...form, destination_url: e.target.value })} />
      <label>Start date/time<input required type="datetime-local" value={form.starts_at} onChange={(e) => setForm({ ...form, starts_at: e.target.value })} /></label>
      <label>End date/time<input required type="datetime-local" value={form.ends_at} onChange={(e) => setForm({ ...form, ends_at: e.target.value })} /></label>
      <label>Priority<input type="number" min="1" max="100" value={form.priority} onChange={(e) => setForm({ ...form, priority: e.target.value })} /></label>
      <label>Budget (GHS)<input type="number" min="0" step="0.01" value={form.budget_minor} onChange={(e) => setForm({ ...form, budget_minor: e.target.value })} /></label>
